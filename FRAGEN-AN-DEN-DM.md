# Fragen an den Spielleiter

Alles, was an Chardex35 auf eine Regelentscheidung wartet — an einer Stelle, zum
Vorlesen oder Weiterschicken.

Warum die Liste hier liegt und nicht im Chat: ein Chat fängt irgendwann von vorn an,
diese Fragen gelten dann trotzdem. Wenn eine Antwort da ist, wird sie in `CLAUDE.md`
eingetragen und hier gestrichen.

**Wie zu lesen:** jede Frage sagt, warum die App die Antwort braucht, und darunter
stehen die Möglichkeiten. Es genügt, eine anzukreuzen — „so wie im Buch" ist auch eine
Antwort.

**Und was solange passiert:** nichts wird blockiert. Die App warnt, sie sperrt nicht —
solange eine Antwort fehlt, bleibt es beim heutigen Verhalten, und das steht bei jeder
Frage unter „Heute".

---

## 1. Trefferpunkte beim Stufenaufstieg

> Philipp: „Dann würfeln wir die TP bei levelup nicht. Was wir da bekommen muss ich
> nochmal fragen."

**Heute:** die App würfelt auf Wunsch (Knopf am Aufstieg), oder sie nimmt den
Durchschnitt und rundet dabei **ab** — beim W8 also 4. Einen festen Wert je Stufe kann
man auch eintippen.

### 1.1 Was gibt es statt des Wurfs?

Die Zahlen ausgeschrieben, damit nichts zu deuten bleibt:

- [ ] **halber Würfel +1** → W4 = 3 · W6 = 4 · W8 = 5 · W10 = 6 · W12 = 7
- [ ] **Durchschnitt abgerundet** → W4 = 2 · W6 = 3 · W8 = 4 · W10 = 5 · W12 = 6
      *(so rechnet die App heute)*
- [ ] **voller Würfel** → W8 = 8
- [ ] **etwas anderes:** ______

*Warum:* jede Stufe jedes Charakters hängt daran. Die App rechnet es dann selbst und
zeigt keinen Würfelknopf mehr, wo nicht gewürfelt wird. Die beiden ersten Möglichkeiten
unterscheiden sich um genau 1 TP pro Stufe — auf Stufe 10 sind das 9 TP.

### 1.2 Gilt für alle Würfelarten dieselbe Regel?

Der Magier hat einen W4, der Barbar einen W12.

- [ ] eine Regel für alle
- [ ] beim W4 mindestens 3
- [ ] eigene Zahl je Würfelart: ______

### 1.3 Gilt das auch für die erste Stufe?

Im Regelwerk bekommt die erste Stufe den vollen Würfel — die App hat dafür einen
Schalter, und der ist standardmäßig AN.

- [ ] erste Stufe bleibt der volle Würfel
- [ ] erste Stufe wie alle anderen
- [ ] voller Würfel auch bei der ersten Stufe einer NEUEN Klasse beim Mehrklassen-Spiel

### 1.4 Gilt die neue Regel rückwirkend?

Auf den Bögen stehen die bisherigen Stufen schon mit ihren Zahlen.

- [ ] nur ab dem nächsten Aufstieg — alles Bisherige bleibt, wie es ist
- [ ] rückwirkend für alle Stufen (Trefferpunkte können dabei auch sinken)
- [ ] rückwirkend, aber nie nach unten: wer mehr hat, behält es

*Warum das eine eigene Frage ist:* rückwirkend heißt, dass sich Zahlen auf fertigen
Bögen verschieben. Das passiert in dieser App nur auf ausdrückliche Ansage.

### 1.5 Was bei negativem Konstitutions-Modifikator?

- [ ] mindestens 1 TP pro Stufe (steht so im Regelwerk, und so rechnet die App heute)
- [ ] es darf auch 0 sein — eine Stufe kann dann nichts bringen

### 1.6 Und die Bögen, die aus Fight Club übernommen wurden?

Auf denen steht die alte TP-Summe als **feste Zahl** (der Import trägt sie ein, damit
nach dem Umzug genau dasselbe auf dem Bogen steht wie vorher). Diese feste Zahl
überstimmt jede Rechnung — auch beim nächsten Aufstieg.

- [ ] die Zahl vom alten Bogen gilt; die App rechnet nur die neuen Stufen dazu
- [ ] die App rechnet alles neu — die Summe darf sich dabei auch nach unten ändern
- [ ] die App rechnet neu, aber nie nach unten: wer mehr hat, behält es

*Warum das nicht dasselbe wie 1.4 ist:* solange die feste Zahl gilt, bewegt sich auf
diesen Bögen gar nichts, egal wie 1.4 ausfällt. Erst diese Antwort macht die neue Regel
auf Torbens Bogen wirksam.

---

## 2. Cantrips bei Vorbereitern (Kleriker, Druide)

> Philipp: „Cantrips müssen nicht vorbereitet werden. Man kann, sofern man noch für
> Level 0 ausreichend zauberpunkte hat, beliebig aus den cantrips wählen. Halt nur für
> vorbereiter (Druide und Kleriker)."

**Heute:** die App verlangt auch auf Grad 0, dass Zauber vorbereitet werden, und meldet
freie Grad-0-Plätze als „noch offen".

### 2.1 Verbraucht ein Cantrip trotzdem einen Grad-0-Platz?

- [ ] ja — die Plätze bleiben, nur die Wahl fällt weg (so klingt Philipps Satz)
- [ ] nein — Cantrips sind unbegrenzt

*Warum:* davon hängt ab, ob die App auf Grad 0 weiter mitzählt oder dort gar keine
Grenze mehr kennt.

### 2.2 Gilt es auch für den Magier?

Genannt sind Druide und Kleriker. Im Regelwerk ist der Magier der dritte, der Zauber
vorbereitet.

- [ ] auch für den Magier
- [ ] nur Kleriker und Druide

### 2.3 Woraus darf frei gewählt werden?

- [ ] aus allen Grad-0-Zaubern der Klasse
- [ ] nur aus denen, die der Charakter kennt (beim Magier: was im Zauberbuch steht)

*Warum:* beim Kleriker ist das dasselbe (er hat die ganze Liste), beim Magier nicht.

### 2.4 Gilt die freie Wahl nur für Grad 0?

- [ ] nur Grad 0
- [ ] auch höhere Grade (dann wären Kleriker und Druide spontane Zauberwirker)

### 2.5 Wie viele Cantrips hat man am Tag?

Mit der neuen Regel ist diese Zahl die **einzige** Grenze für Cantrips — vorher war sie
bloß die Zahl der Zeilen, die man morgens ausfüllt.

- [ ] nur die Zahl aus der Klassentabelle (Kleriker Stufe 1: drei — so rechnet die App)
- [ ] ein hoher WIS bzw. INT gibt auch auf Grad 0 zusätzliche Zauber
- [ ] Grad 0 wird gar nicht mehr gezählt (siehe 2.1)

*Warum:* im Regelwerk geben Bonus-Zauber aus einem hohen Attribut erst ab Grad 1 etwas
her. Wenn das bei euch anders ist, hätte ein Kleriker mit WIS 18 vier statt drei.

### 2.6 Und Barde und Hexenmeister?

Die zaubern ohne Vorbereitung, verbrauchen für einen Cantrip aber weiter einen Platz.

- [ ] auch für sie — Grad 0 kostet dann niemanden mehr einen Platz
- [ ] nein, bei ihnen bleibt es beim Tageslimit
- [ ] die Frage entfällt: Grad 0 bleibt auch bei den Vorbereitern begrenzt, nur die Wahl
      wird frei

*Warum:* falls Grad 0 bei Kleriker und Druide ungezählt wird, steht der Hexenmeister auf
Grad 0 plötzlich schlechter da als sie. Das ist eine Entscheidung für euren Tisch, keine
der App.

---

## 3. Die Spellcraft-Probe statt eines Platzes

> Philipp: „Eine weitere ist, dass wir eine spellcraft Probe machen können um einen
> Zauber ohne einen zauberpunkt zu verbrauchen wirken wollen. Muss ich aber auch
> nochmal beim dm fragen."

**Heute:** kennt die App nicht — wirken heißt Platz verbrauchen.

### 3.1 Welcher Schwierigkeitsgrad?

- [ ] 15 + doppelter Zaubergrad (Grad 3 → 21)
- [ ] 20 + Zaubergrad (Grad 3 → 23)
- [ ] 10 + doppelter Zaubergrad (Grad 3 → 16)
- [ ] fester Wert: ______

### 3.2 Was passiert beim Misslingen?

- [ ] Platz ist weg, Zauber wirkt nicht
- [ ] Platz bleibt, Zauber wirkt nicht (nur die Handlung ist verloren)
- [ ] Zauber wirkt, Platz ist weg (die Probe war nur der Versuch zu sparen)

### 3.3 Wie oft?

- [ ] beliebig oft
- [ ] ______ mal pro Tag
- [ ] einmal pro Zauber und Tag

### 3.4 Für wen?

- [ ] alle Zauberwirker
- [ ] nur Vorbereiter (Kleriker, Druide, Magier)

### 3.5 Muss Spellcraft trainiert sein?

- [ ] ja, mindestens 1 Rang
- [ ] nein, ungeübt geht auch

### 3.6 Braucht man dafür noch einen freien Platz? ← die wichtigste hier

- [ ] **nur mit freiem Platz** — die Probe SPART ihn, das Tageslimit bleibt ein Limit
- [ ] **auch wenn alle Plätze dieses Grades verbraucht sind** — das Tageslimit ist damit
      nach oben offen
- [ ] auch bei Zaubergraden, die die Figur noch gar nicht hat

*Warum das die wichtigste Frage des Abschnitts ist:* bei der ersten Antwort genügt ein
zweiter Knopf neben „Wirken". Bei der zweiten muss die Sperre weg, und der Bogen braucht
eine eigene Zeile für „über dem Tageslimit gewirkt" — sonst weiß nach dem dritten Mal
niemand mehr, wie viel schon über der Grenze liegt.

### 3.7 Welchen Zauber darf man so wirken?

- [ ] nur einen, den man morgens vorbereitet hat
- [ ] jeden aus der Klassenliste
- [ ] beim Magier jeden aus seinem Zauberbuch, bei Kleriker und Druide die ganze Liste

*Warum:* „nur vorbereitet" ist ein Knopf mehr an einer Zeile, die es schon gibt. „Ganze
Klassenliste" heißt: bei einem Kleriker der Stufe 7 stehen über 200 Zauber mit einem
Wirken-Knopf im Bogen, und der braucht dann eine eigene Suche.

### 3.8 Gilt die Probe auch für Cantrips?

- [ ] ja, auch auf Grad 0
- [ ] nein, erst ab Grad 1

*Warum:* hier greifen die zwei neuen Regeln ineinander. Wenn der Schwierigkeitsgrad mit
dem Zaubergrad steigt, wäre die Probe auf Grad 0 die leichteste — Cantrips wären damit
praktisch unbegrenzt, egal wie 2.1 ausfällt. Zwei Regeln, von denen eine die andere
aushebelt.

*Warum die Fragen in diesem Abschnitt so viele sind:* die App soll die Probe anbieten und
den Platz nur dann abziehen, wenn er wirklich weg ist. Ohne 3.2 wüsste sie nicht, was
nach einem misslungenen Wurf passiert; ohne 3.6 nicht, ob das Tageslimit noch gilt.

---

## 4. „Zauberpunkte" oder Zauberplätze?

In den zwei Regeln oben steht zweimal „Zauberpunkte". Die App rechnet mit **Plätzen je
Grad** (drei Grad-0-Plätze, zwei Grad-1-Plätze, …) wie im Regelwerk.

### 4.1 Ist das dasselbe?

- [ ] ja, „Punkt" ist bei uns einfach das Wort für einen Platz
- [ ] nein — wir haben einen echten Vorrat an Punkten, aus dem Zauber aller Grade
      bezahlt werden

*Warum:* das ist der Unterschied zwischen einem anderen Wort und einem anderen
Rechenmodell. Beim zweiten Fall bräuchte die App einen zweiten Weg, Zauber zu
verbuchen — machbar, aber gut zu wissen, bevor der erste gebaut wird.

---

## 5. Aktionspunkte

> Philipp: „Actionpoints hat jeder 6. wann und wie man neue bekommt muss ich nochmal
> abklären beim DM."

**Heute:** man kann sich einen Zähler „Aktionspunkte" selbst anlegen, mit Höchstwert und
frei wählbarer Auffüll-Bedingung. Die 6 und der Nachschub stehen noch nicht als Vorgabe
drin.

### 5.1 Wann kommen sie zurück?

Mehrfach ankreuzen ist erlaubt.

- [ ] nach einer langen Rast (acht Stunden)
- [ ] **auch bei einer kurzen Pause** (eure Hausregel — die App erkennt sie sicher)
- [ ] beim Stufenaufstieg
- [ ] pro Spielabend
- [ ] nur wenn der Spielleiter welche gibt
- [ ] anders: ______

*Warum:* die App kann Zähler bei langer Rast, kurzer Pause und Aufstieg von allein
zurücksetzen — aber nur, wenn sie weiß, bei welcher Gelegenheit. „Pro Spielabend" kann
sie nicht erkennen; dann bliebe ein Knopf zum Selbst-Auffüllen. Der Unterschied am Tisch
ist groß: bei „kurze Pause" stehen die Punkte mehrmals am Abend wieder da.

### 5.2 Bleiben es 6?

- [ ] immer 6, über alle Stufen
- [ ] 5 + halbe Charakterstufe, abgerundet (die verbreitete Fassung — auf Stufe 2 wären
      das genau 6, und dann würde die Zahl weiter wachsen)
- [ ] wächst anders: ______

### 5.3 Verfallen übrige Punkte?

- [ ] übrige Punkte verfallen beim Auffüllen
- [ ] sie werden mitgenommen (dann kann man mehr als 6 haben)

### 5.4 Wer trägt sie ein, wenn du welche vergibst?

- [ ] jeder Spieler an seinem eigenen Bogen
- [ ] der Spielleiter setzt sie an den Bögen der Spieler

*Warum:* in dieser App besitzt der Spielleiter den AUFBAU eines Bogens (Stufen, Talente,
Ausrüstung) und der Spieler den SPIELZUSTAND (Schaden, verbrauchte Zauber, Zähler).
Aktionspunkte sind Spielzustand — der Weg vom Spielleiter zum Zähler eines Spielers ist
heute also zu. Wenn du sie setzen willst, muss dafür ein Weg gebaut werden.

---

## 6. Zweite Hand: halber Stärkeschaden?

Steht schon länger offen.

**Heute:** die zweite Hand bekommt den **vollen** Stärkebonus auf den Schaden.

### 6.1 Nur der halbe Bonus in der zweiten Hand?

Beispiel Dolch (W4) mit STR 14 (+2):

- [ ] 1d4+1 — halber Bonus (so steht es im Regelwerk)
- [ ] 1d4+2 — voller Bonus (so rechnet die App heute)

*Warum:* das verschiebt Zahlen an bestehenden Bögen, deshalb wird ohne Antwort nichts
angefasst.

---

## 7. Zweihandwaffe bei negativer Stärke

**Heute:** die App rechnet den Anderthalbfach-Weg auch bei einem Malus und rundet nach
unten — STR 8 (−1) mit einer Zweihandwaffe ergibt also **−2** auf dem Schaden.

### 7.1 STR 8 (−1), Zweihandwaffe — was steht auf dem Schaden?

- [ ] **−1** — ein Malus wird nicht vervielfacht (so liest sich das Regelwerk: der
      Anderthalbfach-Weg gilt für einen BONUS)
- [ ] **−2** — auch der Malus zählt anderthalbfach, nach unten gerundet (so rechnet die
      App heute)
- [ ] **−1** — anderthalbfach, aber Richtung 0 gerundet

*Warum:* betrifft nur Charaktere mit STR 8 oder weniger und eine Zweihandwaffe — selten,
aber die Zahl steht dann falsch auf dem Bogen.

---

## 8. Volle Attacke ab Grundangriffsbonus +6

**Heute:** der Bogen zeigt die ganze Reihe, genau wie Fight Club — bei
Grundangriffsbonus +9 also „+9/+4". Bei der zweiten Hand zeigt er nur die Angriffe, die
die Zweiwaffen-Talente hergeben, nicht die Reihe.

### 8.1 Ist das für euren Tisch richtig?

- [ ] ja, wie im Regelwerk (zweiter Angriff ab +6, dritter ab +11, jeder weitere −5)
- [ ] nein, bei uns gibt es einen Angriff pro Runde — die zweite Zahl kann weg
- [ ] anders: ______

*Warum:* wenn ihr die volle Attacke nicht spielt, steht auf jedem Bogen ab Stufe 6 eine
Zahl, die niemand benutzt. Und weil es JEDEN Bogen der Gruppe betrifft, wird daran ohne
Antwort nichts angefasst.

---

## 9. Drei Regeln, für die schon ein Fach existiert

Beim Durchsehen der Hausregel-Einstellungen kam heraus: die App hat für drei Tischregeln
ein Feld, aber keine davon tut etwas. Zwei stehen nicht einmal in den Einstellungen, und
eine hat einen Schalter, der nichts bewirkt. Das sind billige Fragen mit einer fertigen
Hälfte dahinter.

### 9.1 Erfahrungspunkte-Strafe beim Mischen von Klassen?

Die Regel im Buch: liegen die Klassenstufen einer Figur mehr als eine Stufe auseinander,
gibt es weniger Erfahrungspunkte.

- [ ] spielen wir nicht → der Schalter kann verschwinden
- [ ] spielen wir → dann soll der Bogen es melden

*Heute:* der Schalter steht in den Einstellungen und lässt sich umlegen, aber **keine
Zeile der App liest ihn**. Er verspricht etwas und tut nichts — das ist schlimmer als
gar kein Schalter.

### 9.2 Ab wann ist eine Figur tot?

- [ ] bei −10 Trefferpunkten (Regelwerk)
- [ ] bei so vielen Minuspunkten, wie sie Konstitution hat (CON 14 → tot bei −14)
- [ ] andere Grenze: ______
- [ ] gar nicht anzeigen, das sagt der Spielleiter am Tisch

*Heute:* die TP dürfen negativ werden, aber die App sagt nie „bewusstlos" oder „tot".

### 9.3 Wie entstehen die Attributswerte?

- [ ] gewürfelt
- [ ] mit einem Punktebudget gekauft, und zwar ______ Punkte
- [ ] der Spielleiter gibt sie vor

*Heute:* der Assistent nimmt sechs Zahlen und prüft nichts. Bei einem Budget könnte er
mitzählen und warnen, wenn es überzogen ist.

---

## Schon geklärt — bitte nicht neu fragen

Damit die Liste nicht länger wird als sie muss:

- **Zweiwaffen-Mali gelten** an diesem Tisch (−6/−10, mit leichter Zweitwaffe und
  Talent entsprechend geringer).
- **Untote vertreiben: 7** beim aktuellen Kleriker (3 + CHA + 4 aus dem Talent).
- **Die kurze Pause** ist eine Hausregel dieses Tisches: sie füllt die Tageszähler,
  ohne acht Stunden zu brauchen. Zauberplätze fasst sie nicht an.
- **Zwei Domänen** mit je einem Domänenplatz pro Zaubergrad ab Grad 1 (auf Grad 0 gibt
  es keinen Domänenplatz).
- **Eine Rast fasst die TP nicht an**, und temporäre TP überdauern eine Nacht.
